    ...
    const handleReschedule = async () => {
        ...
        try {
            await axiosInstance.put(
                userEndPoints.USER.RESCHEDULE_APPOINTMENT,
                {
                    appointmentId,
                    newSlotDate: selectedDay.value,
                    newSlotTime: selectedTime.label,
                }
            );
            ...
            <div className="bg-blue-regBg min-h-screen p-6 flex flex-col items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 mt-6 md:gap-72 max-w-7xl w-full">
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-4 gap-6 md:w-[730px]">
                            ...
                            <div className="flex flex-wrap gap-4 mt-2">
                                {availableDays.map((day, index) => (
                                    <button 
                                        ...
                                        className={`px-5 py-2 rounded-full text-white shadow-md ${
                                            selectedDay?.value === day.value ? "bg-red-500" : "bg-blue-default"
                                        }`}
                                    >
                                        {day.label}
                                    </button>
                                ...
                                <div className="flex flex-wrap gap-3 mt-2">
                                    {availableTimeSlots.map((time, index) => (
                                        <button 
                                            ...
                                            className={`px-3 py-2 rounded-full text-white shadow-md ${
                                                selectedTime?.value === time.value ? "bg-red-500" : "bg-blue-default"
                                            }`}
                                        >
                                            {time.label}
                                        </button>
                        ...
                        <button 
                            onClick={isRescheduling ? handleReschedule : handleBooking} 
                            className="mt-6 w-full bg-blue-default hover:bg-blue-700 text-white py-3 font-semibold rounded-full shadow-md"
                        >
                            {isRescheduling ? "Confirm Reschedule" : "Schedule"}
                        </button>
                    </div>
